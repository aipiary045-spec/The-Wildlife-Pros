...
test("work orders land in a pipeline bucket", () => {
  ...
  assert.equal(workOrderBucket({ status: "COMPLETED", scheduledStart: new Date(2026, 7, 16, 9, 0, 0) }, now), "closed");
...
test("technician done view labels finished work", () => {
  ...
  const sections = groupWorkOrders(jobs, done, now);

...
import { flushOfflineQueue, pendingMutationCount, subscribeOfflineQueue } from "@/lib/field-fetch";
  ...
  return (
    <div className="border-b border-amber-200 bg-amber-50 px-4 py-2 text-sm text-ink md:px-6">
      {!online ? (
        ...
        <p>
          {uploading ? "Uploading saved field work…" : `${pending} saved action${pending === 1 ? "" : "s"} waiting to upload.`}
        </p>
      ) : null}
    </div>
  );

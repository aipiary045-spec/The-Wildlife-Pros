...
- Route math in `src/lib/routing.ts` (Haversine or Mapbox Matrix road costs, nearest-neighbor, 2-opt). Persist/Prisma lives in `src/lib/route-plan.ts`. Map URLs in `src/lib/maps.ts`; geocoding / Mapbox Matrix + Directions in `src/lib/geocode.ts`. Route map preview: `src/components/routes/RouteMap.tsx` (MapLibre + free Carto tiles).
